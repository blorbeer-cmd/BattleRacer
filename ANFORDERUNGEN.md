...
| **A-011** | 🔴 | 🔄 | **Raycast-Kart-Controller:** `RigidBody3D` mit Raycast-Federung; Gas, Bremse, Rückwärts, analoge Lenkung; sichtbares Kart-Mesh wird rein optisch nachgeführt (Neigung in Kurven, Federweg). Im ersten Gamepad-/Tastatur-Playtest bestätigt: Gas/Bremse/Lenkung funktionieren vorwärts; zwei Bugs gefunden und behoben (Lenkung zu schwach, Lenkung beim Rückwärtsfahren seitenverkehrt) — weitere Feinabstimmung des Fahrgefühls steht noch aus |
...
| **A-013** | 🔴 | 🔄 | **Drift + Mini-Turbo:** Drift-Knopf mit Hop, gehaltener Drift lädt Funken-Stufen (2 Stufen wie MK64), Boost beim Lösen; Drift-Richtung unabhängig von Lenk-Feineinstellung — Hop-Impuls und Farb-Rückmeldung je Mini-Turbo-Stufe vorhanden. Bug behoben: Lenkung fror während des kurzen Hop-Luftmoments komplett ein (spürbar als „zieht kurz gerade"), da `_apply_steering` an Bodenkontakt gekoppelt war — Kopplung entfernt. Lenkempfindlichkeit generell als zu hoch gemeldet und gedämpft. **Timing/Balance braucht weiterhin echtes Spielgefühl-Feedback**, insbesondere von jemandem mit MK64-Referenzerfahrung |
| **A-014** | 🔴 | ✅ | **Verfolgerkamera:** Weiche Kamera hinter dem Kart, zieht bei Boost leicht auf (FOV), `look_back`-Taste für Rückblick — im Playtest bestätigt: Kamera folgt korrekt |
